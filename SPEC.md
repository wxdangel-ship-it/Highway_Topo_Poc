...
3) 明确：允许用 **非坐标的索引化位置（bin 区间）**做问题定位；禁止任何坐标本身。
...
- 外传任何信息（尤其锚点定位）禁止包含坐标本身，只能用非坐标索引方式定位（见第 9 章）
...
- 可诊断：每个技术点必须输出分位数、异常区间统计、失败原因枚举；外网仅凭文本可定位问题类型与大致发生位置（非坐标）
...
### 4.3 脱敏与安全假设
- 内网回传外网的任何内容不包含：坐标/原始几何/图片/点云片段/内网路径
- 回传允许：统计分位数、区间长度占比、匿名 ID、失败原因枚举、候选排序摘要、输入类型/分辨率等元信息
- 允许使用非坐标索引（bin 区间）辅助定位（见第 9 章）
...
- 分位数：p50/p90/p99（方案A）
- 异常区间：沿标量轴连续超阈区间（外传只允许 bin 区间）
- bin 区间：将标量轴离散化后的索引区间，用于定位（非坐标）
...
- 运行环境：WSL + Python
- 工作区约束：项目必须放在 Windows E: 盘（WSL 下通常 /mnt/e/...）；外传文本禁止出现绝对路径
...
## 14. 隐私与脱敏要求（内网侧）
禁止回传外网：
- 任何坐标（任何形式）、任何几何顶点数组、任何图片/点云片段、任何内网路径/机器信息
允许回传：
- 分位数、阈值、计数/比例、bin 区间、失败原因枚举、输入类型/分辨率摘要
...
provider:
  type: "file_patch"
  data_root: "/mnt/e/..."        # 内网本地路径示意；外传文本禁止出现该类路径
