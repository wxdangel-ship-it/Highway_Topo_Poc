...
def _merge_patch_rows(patch_dir: Path) -> list[dict[str, Any]]:
    global_fit_payload = _read_json(patch_dir / "step5_global_fit_trace.json")
        ...
        rows.append(
            {
                ...
                "trajectory_spine_weak_bool": bool(item.get("trajectory_spine_weak_bool", False)),
                "trajectory_spine_fallback_bool": bool(item.get("trajectory_spine_fallback_bool", False)),
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="t05_extract_global_fit_trace")
