...
from highway_topo_poc.modules.t05_topology_between_rc_v2.review import write_step5_finish_review


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="t05_v2_step5_finish_55353246_37687913_review")
    parser.add_argument("--run-root", required=True, help="Directory that contains patches/<patch_id>/...")
    parser.add_argument(
        "--output-root",
        help="Defaults to <repo>/outputs/_work/t05_v2_step5_finish_55353246_37687913_<timestamp>",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = Path(args.output_root) if args.output_root else (
        repo_root / "outputs" / "_work" / f"t05_v2_step5_finish_55353246_37687913_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
