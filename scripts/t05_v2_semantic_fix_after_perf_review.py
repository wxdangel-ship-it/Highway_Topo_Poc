...
from highway_topo_poc.modules.t05_topology_between_rc_v2.review import write_semantic_fix_after_perf_review


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="t05_v2_semantic_fix_after_perf_review")
    parser.add_argument("--run-root", required=True, help="Directory that contains patches/<patch_id>/...")
    parser.add_argument(
        "--output-root",
        help="Defaults to <repo>/outputs/_work/t05_v2_semantic_fix_after_perf_<timestamp>",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_root = Path(args.output_root) if args.output_root else (
        repo_root / "outputs" / "_work" / f"t05_v2_semantic_fix_after_perf_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    )
