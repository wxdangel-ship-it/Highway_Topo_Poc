...
CMD=("$PY" -m highway_topo_poc.modules.t05_topology_between_rc_v2.run --data_root "$DATA_ROOT" --patch_id "$PATCH_ID" --run_id "$RUN_ID" --out_root "$OUT_ROOT" --stage step6_build_road)
if [ "$DEBUG" = "1" ]; then
  CMD+=(--debug)
fi
if [ "$FORCE" = "1" ]; then
